...
跨类型排序（记忆 vs 知识库）是个难题。与其做复杂的排序算法，不如把结果都给 LLM，让它判断哪些有用。

**实现示例**：

```go
func (s *SearchService) Search(ctx context.Context, userID, query string) (*SearchResult, error) {
    var memResults, kbResults []Result

    g, ctx := errgroup.WithContext(ctx)

    g.Go(func() error {
        memResults, _ = s.memory.Search(ctx, userID, query, limit: 3)
        return nil
    })

    g.Go(func() error {
        kbResults, _ = s.knowledge.Search(ctx, userID, query, limit: 3)
        return nil
    })
...
| 场景 | 处理方式 |
|------|---------|
| 上传文档让 AI 了解我 | 对话中提取 → 更新画像 |
| 上传文档作为参考资料 | 使用知识库（已有） |
| 一次性任务附件 | 仅在当前对话使用 |

**核心原则**：记忆系统专注于对话，不直接存储文件。

### 对话中提取个人信息

当用户在对话中发送了关于自己的文档（如简历）时：

```
用户：[上传 resume.pdf] 帮我看看这份简历
AI：（正常处理任务）
AI：我注意到你有5年后端经验，在XX公司工作。要我记住这些信息吗？
用户：好的
AI：（提取关键信息 → 更新核心画像）
```
...
```go
// 对话结束时，检查是否有可提取的个人信息
func (s *Summarizer) ExtractProfileUpdates(ctx context.Context, messages []Message) (*ProfileUpdate, error) {
    // 检查对话中是否涉及用户个人信息
    // 如果有，提取并返回建议的更新
    // 由上层决定是否询问用户确认
}
...
如果发现用户经常需要通过文档建立画像，可考虑：
- 添加专门的"个人资料"上传入口
- 支持批量导入个人文档
- 自动分析并生成初始画像

但当前阶段不实现，遵循 YAGNI 原则。
